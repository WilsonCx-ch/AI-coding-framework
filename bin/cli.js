	...
	return {
		mcpServers: {
			...
			pencil: {
				...
				transport: 'stdio',
				command:
					'安装 Pencil VS Code/Cursor 插件后，在扩展目录中查找 mcp-server 路径并填写于此',
				args: ['--app', 'cursor'],
				env: {}
			}
		}
...
安装脚手架时会自动将 \`rules\`、\`skills\` 链接到项目根目录下的 \`.agents\`（软链失败时会回退为复制目录），以便与 \`.agents\` 唯一维护的规范与技能保持一致。
`
}
...
function ensureDirAlias(linkPath, targetPath) {
	const sourceExists = fs.existsSync(targetPath)
	...
	if (fs.existsSync(linkPath)) {
		...
		if (stat.isSymbolicLink()) {
			const linkedReal = safeRealpath(linkPath)
			const targetReal = safeRealpath(targetPath)
			if (linkedReal === targetReal) return
		}
	...
	try {
		...
		fse.copySync(targetPath, linkPath)
	}
}

function linkEditorAgents(targetDir, editorDirName) {
	const editorRoot = path.join(targetDir, editorDirName)
	const agentsRoot = path.join(targetDir, '.agents')
	ensureDirAlias(path.join(editorRoot, 'rules'), path.join(agentsRoot, 'rules'))
	ensureDirAlias(
		path.join(editorRoot, 'skills'),
...
`)
	...
	const targetDirInput = path.resolve(process.cwd(), rawDir || '.')
	...
	const targetDir = safeRealpath(targetDirInput)
	...
	for (const editor of editors) {
		linkEditorAgents(targetDir, `.${editor}`)
	}
